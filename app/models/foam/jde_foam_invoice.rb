class Foam::JdeFoamInvoice < ActiveRecord::Base
  ...
  def self.import_sales(date)
    invoices = find_by_sql("SELECT SA.RPLNID AS LINEFAKTUR, SA.RPDOC AS NOFAKTUR, SA.RPDCT AS ORTY, SA.RPSDOC AS NOSO, SA.RPSDCT AS DOC, SA.RPSFX AS LINESO, 
       ...
       (
         SELECT * FROM PRODDTA.F03B11 WHERE RPDIVJ = '120293' 
         AND REGEXP_LIKE(rpdct,'RI|RO|RX') AND REGEXP_LIKE(rppost,'P|D') AND RPAN8 = '100171'
       ) SA
       LEFT JOIN
       (
       SELECT * FROM PRODDTA.F4101 WHERE IMTMPL LIKE '%BUSA%' -- item master
       ) IM ON TRIM(IM.IMLITM) = TRIM(SA.RPRMK)
       LEFT JOIN
       (
       SELECT * FROM PRODDTA.F0006 -- BP DETAIL
       ) MC ON MC.MCMCU LIKE '%'||TRIM(SA.RPMCU)
       LEFT JOIN
       (
       SELECT DRKY, DRDL01 FROM PRODCTL.F0005 WHERE DRSY = '00' AND DRRT = '04' -- BRANCH DESCRIPTION
       ) CB ON TRIM(CB.DRKY) LIKE '%'||TRIM(MC.MCRP04)
       LEFT JOIN
       (
       SELECT DRKY, DRDL01 FROM PRODCTL.F0005 WHERE DRSY = '41' AND DRRT = 'S1' -- BRAND
       ) BR ON TRIM(BR.DRKY) LIKE '%'||TRIM(IM.IMSRP1)
       LEFT JOIN
       (
       SELECT DRKY, DRDL01 FROM PRODCTL.F0005 WHERE DRSY = '41' AND DRRT = 'S2' -- SUB BRAND
       ) SBR ON TRIM(SBR.DRKY) LIKE '%'||TRIM(IM.IMSRP2)
       LEFT JOIN
       (
       SELECT DRKY, DRDL01 FROM PRODCTL.F0005 WHERE DRSY = '56' AND DRRT = 'TI' -- TIPE DESCRIPTION
       ) JN ON TRIM(JN.DRKY) LIKE '%'||TRIM(IM.IMSEG2)
       LEFT JOIN
       (
       SELECT DRKY, DRDL01 FROM PRODCTL.F0005 WHERE DRSY = '55' AND DRRT = '08' -- DENSITY DESCRIPTION
       ) DE ON TRIM(DE.DRKY) LIKE '%'||TRIM(IM.IMSEG3)
       LEFT JOIN
       (
       SELECT DRKY, DRDL01 FROM PRODCTL.F0005 WHERE DRSY = '56' AND DRRT = 'FB' -- FEEL BUSA DESCRIPTION
       ) FE ON TRIM(FE.DRKY) LIKE '%'||TRIM(IM.IMSEG4)
       LEFT JOIN
       (
       SELECT DRKY, DRDL01 FROM PRODCTL.F0005 WHERE DRSY = '56' AND DRRT = 'FI' -- FITUR BUSA DESCRIPTION
       ) FI ON TRIM(FI.DRKY) LIKE '%'||TRIM(IM.IMSEG5)
       LEFT JOIN
       (
       SELECT DRKY, DRDL01 FROM PRODCTL.F0005 WHERE DRSY = '56' AND DRRT = 'WB' -- FITUR BUSA DESCRIPTION
    ...
    invoices.each do |iv|
      ...
      month = julian_to_date(iv.tanggalinvoice).to_date.month
        check = Foam::SalesFoamBase.find_by_sql("SELECT nofaktur, orty, lnid, harganetto2 FROM dbmarketing.tblaporancabang 
        ...
        if check.empty?
          ...
          Foam::SalesFoamBase.create!(cabang_id: iv.branchid.strip, cabang_desc: iv.branchdesc.strip, noso: iv.nofaktur.to_i, tanggalsj: julian_to_date(iv.tanggalinvoice),
            ...
            brand: iv.brand.strip, subbrand: iv.subbrand.strip, tipeid: iv.tipe.strip, tipedesc: iv.namatipe.strip, 
            densityid: iv.densityid.strip, densitydesc: iv.densitydesc.strip, feelid: iv.feelid.strip, 
            feeldesc: iv.feeldesc.strip, fiturid: iv.fiturid.strip, fiturdesc: iv.fiturdesc.strip,
            warnaid: iv.warnaid.strip, warnadesc: iv.warnadesc.strip ,panjang: iv.panjang, lebar: iv.lebar, 
            tebal: iv.tebal.strip,
