class AsongOrder < ActiveRecord::Base
  ...
  def creating_point
    check_stock = checking_stock(self.short_item, self.branch_plant).map(&:lilotn)
    aln = AsongLotNumber.find_by_sql("SELECT lot_number FROM asong_lot_numbers WHERE
    short_item = '#{self.short_item}' and branch = '#{self.branch_plant}'").map(&:lot_number)
    check_stock -= aln
    if check_stock.present?
      ...
      val_params.each do |a|
        al = AsongLotNumber.create!(asong_order_id: self.id, lot_number: a, branch: self.branch_plant,
