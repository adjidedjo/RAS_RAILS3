class AccountingPriceListsController < ApplicationController
  ...
  def export_excel
    @export = LaporanCabang.search_by_branch(params[:branch]).brand_on_kodebrg(params[:brand]).month(params[:date][:month]).year(params[:date][:year]) if params[:date].present?
